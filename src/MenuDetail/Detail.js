...
const Detail = ({cart, setCart}) => {      
    ...
    const handleQuantitiy =(type)=> {
       ...
       const handleCart =() =>{
         ...
         if(found) setQuantity(cartItem.title, found.quantity + count) // 중복된 메뉴 확인 -> 해당 메뉴 수량 증가
         else{
        ...
        <div className='detail_wrap'>
            <div className='left'>
                <div className='food_img'>
                    <img src={process.env.PUBLIC_URL + menu.img} width='630px' height='630px'></img>
                </div>
            </div>
            <div className='right'>
                <div className='food_info'>
                    <p className='food_title'>{menu.category}-{menu.title}</p>
                ...
                <div className='quantity'>
                    ...
                    <div className='amount'>
                        <button className='minus' onClick={()=> handleQuantitiy("minus")}>-</button>
                        <div className='count'>
                            <span>{count}</span>
                        </div>
                        <button className='plus' onClick={()=> handleQuantitiy("plus")}>+</button>
                    </div>
                </div>
